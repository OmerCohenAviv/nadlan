...
INPUT_DIR = "successful_neighborhoods"

# Exclude summary/all-cities files
def is_city_csv(filename):
    return filename.endswith('_neighborhoods.csv') and not filename.startswith('all_cities')
    ...
    while len(all_transactions) < max_transactions:
        ...
        try:
            ...
            if response_text:
                ...
                if not transactions:
                    break
                # Filter only residential transactions
                residential_transactions = [t for t in transactions if is_residential(t.get('dealNature', ''))]
                for transaction in residential_transactions:
...
def main():
    for filename in os.listdir(INPUT_DIR):
        if not is_city_csv(filename):
            continue
        city = filename.split('_', 1)[-1].rsplit('_', 1)[0]
        output_dir = f"{city}_neighborhoods_residential"
        ...
        with open(os.path.join(INPUT_DIR, filename), 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                print(f"🔍 Fetching transactions for {city} - {neighborhood_name} ({neighborhood_id})...")
                ...
                with open(output_path, 'w', encoding='utf-8') as out_f:
                    json.dump(node, out_f, ensure_ascii=False, indent=2)
                print(f"💾 Saved {len(transactions)} residential transactions to {output_path}")
                time.sleep(2)
